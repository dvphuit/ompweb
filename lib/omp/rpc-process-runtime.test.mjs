...
const { encodeInboundRpcChunks, MAX_RPC_FRAME_BYTES, RpcFrameTooLargeError } = jiti("./rpc-frame.ts");
...
test("RpcProcess negotiates v2 and correlates out-of-order command responses", async () => {
  ...
  // Writes are serialized through a FIFO queue (backpressure on one command
  // must not corrupt the next), so the commands land asynchronously.
...
test("RpcProcess reassembles v2 events and rejects pending commands when the child crashes", async () => {
  ...
  transport.sendFrames(encodeInboundRpcChunks(expected, "event-1"));
...
test("RpcProcess fails fast on oversized outbound frames and never writes them", async () => {
  const transport = makeTransport();
  transport.onCommand = (command) => {
    if (command.type === "negotiate_protocol") {
      transport.send({ type: "response", id: command.id, command: command.type, success: true, data: { protocolVersion: 2 } });
  ...
  const commandsBefore = transport.commands.length;
  // A ~1.2 MiB base64 image is exactly the reported production failure.
  const oversized = { type: "prompt", message: "", images: [{ type: "image", mimeType: "image/png", data: "A".repeat(MAX_RPC_FRAME_BYTES + 200_000) }] };
  await assert.rejects(
    proc.sendCommand(oversized, 5_000),
    (error) => {
      assert.ok(error instanceof RpcFrameTooLargeError || error.name === "RpcFrameTooLargeError");
      return true;
    },
  );
  // Nothing was written to omp stdin: no rpc_chunk garbage, no hung command.
  assert.equal(transport.commands.length, commandsBefore);
  for (const command of transport.commands) {
    assert.notEqual(command.type, "rpc_chunk");
  }

  // The process stays usable: a normal-sized command is still accepted and acked.
  transport.onCommand = (command) => {
    if (command.type === "negotiate_protocol") {
      transport.send({ type: "response", id: command.id, command: command.type, success: true, data: { protocolVersion: 2 } });
    } else if (command.type === "ping") {
      transport.send({ type: "response", id: command.id, command: "ping", success: true, data: "pong" });
    }
  };
  assert.equal(await proc.sendCommand({ type: "ping" }), "pong");
  await proc.dispose(0);
});
