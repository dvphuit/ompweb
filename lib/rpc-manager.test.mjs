...
test("live session state uses one shared get_state helper", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const helper = source.slice(source.indexOf("export async function getLiveRpcSessionState"));
  assert.match(helper, /getRpcSession\(sessionId\)/);
  assert.match(helper, /return \{ running: false \}/);
  assert.match(helper, /session\.send\(\{ type: "get_state" \}\)/);
});
...
test("set_model restarts an idle child once when its model registry is stale", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const setModel = source.slice(source.indexOf('case "set_model": {'), source.indexOf('case "set_fast_mode": {'));

  assert.match(setModel, /error instanceof RpcCommandError/);
  assert.match(setModel, /model not found/i);
  assert.match(setModel, /this\.isRunning\(\)/);
  assert.match(setModel, /await this\.restart\(\)/);
  assert.equal((setModel.match(/type: "set_model"/g) ?? []).length, 2, "must retry set_model exactly once");
});
...
test("outbound frames over 1 MiB fail fast with code frame_too_large and are never written", async () => {
  const { createJiti } = await import("jiti");
  const jiti = createJiti(import.meta.url);
  const { AgentSessionWrapper } = jiti("./rpc-manager.ts");
  ...
  const fakeProc = {
    ...
    sendCommand: (command) => {
      ...
      return Promise.resolve({ agentInvoked: true });
    },
    sendFrame: () => {},
    dispose: async () => {},
  };
  ...
  // ~950 KiB of prose plus 90 KiB of base64 image data serializes to a frame
  // over the 1 MiB stdin limit while staying well under the 8 MiB HTTP cap.
  const oversized = {
    type: "prompt",
    message: "x".repeat(950 * 1024),
    images: [{ type: "image", mimeType: "image/png", data: "YWJj".repeat(30_000) }],
  };
  await assert.rejects(
    () => wrapper.send(oversized),
    (err) => {
      assert.equal(err.name, "WebRpcError");
      assert.equal(err.code, "frame_too_large");
      return true;
    },
  );
  // Nothing was written to omp: no undeliverable frame, no hung prompt, and
  // the wrapper never entered the prompt-running state.
  assert.equal(sentCommands.length, 0);
  assert.equal(wrapper.isRunning(), false);

  // A normal-sized command on the same wrapper still goes through.
  const normal = wrapper.send({ type: "set_thinking_level", level: "low" });
  await assert.doesNotReject(normal);
  assert.ok(sentCommands.some((c) => c.type === "set_thinking_level"));

  // Release the wrapper's idle timer so the test process exits promptly.
  await wrapper.destroyAndWait();
});

test("prompt ack timeout recycles the child but first emits a terminal agent_end over SSE", async () => {
  const { createJiti } = await import("jiti");
  const jiti = createJiti(import.meta.url);
  const { AgentSessionWrapper } = jiti("./rpc-manager.ts");
  const { RpcCommandTimeoutError } = jiti("./omp/rpc-process.ts");
  ...
  const fakeProc = {
    ...
    onFrame: () => () => {},
    sendCommand: (command, timeoutMs) =>
      Promise.reject(new RpcCommandTimeoutError(command.type, timeoutMs)),
    sendFrame: () => {},
    dispose: async () => { fakeProc.disposed = true; fakeProc.isAlive = false; },
  };
  ...
  await assert.rejects(
    ...
    (err) => err.code === "session_unresponsive",
  );

  // The SSE observer (other tabs included) must see the run terminate
  // immediately rather than spinning until its reconcile poll.
  assert.ok(events.some((e) => e.type === "agent_end" && e.isTerminal === true),
    `expected terminal agent_end, got: ${JSON.stringify(events.map((e) => e.type))}`);
  assert.ok(events.some((e) => e.type === "notice" && e.level === "error"));
  assert.equal(fakeProc.disposed, true, "wedged child must be recycled");
  assert.equal(wrapper.isAlive(), false);
});

test("outbound size guard rejects before any command is dispatched", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  assert.match(source, /assertOutboundFrameFits\(command\)/);
  assert.match(source, /MAX_RPC_FRAME_BYTES/);
  assert.match(source, /"frame_too_large"/);
  // rpc-process writes exactly one JSONL line per outbound frame and never
  // emits rpc_chunk on stdin.
  const processSource = await readFile(new URL("./omp/rpc-process.ts", import.meta.url), "utf8");
  assert.doesNotMatch(processSource, /type: "rpc_chunk"/);
  const frameSource = await readFile(new URL("./omp/rpc-frame.ts", import.meta.url), "utf8");
  assert.match(frameSource, /class RpcFrameTooLargeError/);
  assert.match(frameSource, /stdin has no chunk reassembler|one JSON object per line/);
});
