...
test("drops empty thinking even while streaming", async () => {
  const { splitFinalAssistantBlocks, getDisplayableAssistantBlocks } = await loadSubject();
  const message = assistant([
    { type: "thinking", thinking: "" },
  ...
  assert.deepEqual(split.answerBlocks.map((block) => block.type), ["text"]);
  assert.deepEqual(split.processBlocks, []);
  assert.deepEqual(getDisplayableAssistantBlocks(message, { isStreaming: true }).map((b) => b.type), ["text"]);
});
