...
test("live-state routes share the canonical helper and inline state is best effort", async () => {
  const agentRoute = await readFile(new URL("../app/api/agent/[id]/route.ts", import.meta.url), "utf8");
  const sessionRoute = await readFile(new URL("../app/api/sessions/[id]/route.ts", import.meta.url), "utf8");
  const stateRoute = await readFile(new URL("../app/api/sessions/[id]/state/route.ts", import.meta.url), "utf8");
  assert.match(agentRoute, /getLiveRpcSessionState\(id\)/);
  assert.match(sessionRoute, /agent = await getLiveRpcSessionState\(id\)/);
  assert.match(sessionRoute, /Leave agent unset/);
  assert.match(stateRoute, /await getLiveRpcSessionState\(id\)/);
  assert.match(stateRoute, /resolveSessionPathOr404\(id\)/);
});
...
test("OMP update route permits check and restart actions with force support", async () => {
  ...
  assert.match(settings, /fetch\("\/api\/omp-update"[\s\S]*?action:\s*"check"/);
  assert.doesNotMatch(appShell, /force:\s*true/);
});

test("settings exposes manual update controls without checking on app load", async () => {
  const settings = await readFile(new URL("../components/SettingsConfig.tsx", import.meta.url), "utf8");
  ...
  assert.match(appShell, /"\/api\/app-update\?force=1"/);
  assert.match(settings, /fetch\("\/api\/omp-update/);
  assert.doesNotMatch(settings, /hasCheckedUpdates/);
  // Update checks are manual-only: nothing fetches on app load or tab open —
  // refreshes happen only through the Refresh buttons / dialogs.
  assert.doesNotMatch(appShell, /fetch\("\/api\/omp-update"/);
  assert.doesNotMatch(appShell, /refreshAppUpdate\(false, true\)/);
...
test("agent routes bound requests with the shared attachment budget", async () => {
  ...
  // Below Next's 10 MB proxy buffering boundary; HTTP body budget only binds
  // for non-image payloads (image frames bind at the 1 MiB omp wire limit).
  assert.match(budget, /MAX_AGENT_COMMAND_REQUEST_BYTES = 8 \* 1024 \* 1024/);
  // The aggregate image budget is derived from the 1 MiB omp stdin frame
  // limit (one JSON object per line, no inbound chunk reassembly).
  assert.match(budget, /MAX_AGENT_WIRE_FRAME_BYTES = 1024 \* 1024/);
  assert.match(budget, /MAX_TOTAL_ATTACHED_IMAGE_BYTES = Math\.floor\(/);
});
