...
test("validateProjectPath rejects empty, missing, and non-directory paths", (t) => {
  ...
  assert.equal(validateProjectPath(dir), realpathSync(dir));
});
...
test("worktree paths canonicalize to the main repo root when registered", async (t) => {
  ...
  try {
    ...
    assert.equal(project.projectRoot, realpathSync(repo));
    ...
    assert.equal(registered.projects[0].path, realpathSync(repo));
    ...
    assert.deepEqual(merged.map((p) => p.path), [realpathSync(repo)]);
