...
test("allows equivalent loopback hosts but rejects wildcard bind addresses", async () => {
  const { isApiRequestOriginAllowed } = await loadSubject();
  assert.equal(isApiRequestOriginAllowed(new Request("http://localhost:30141/api/test", {
    headers: { host: "127.0.0.1:30141", origin: "http://localhost:30141" },
  })), true);
  assert.equal(isApiRequestOriginAllowed(new Request("http://localhost:30141/api/test", {
    headers: { host: "0.0.0.0:30141", origin: "http://localhost:30141" },
  })), false);
});
