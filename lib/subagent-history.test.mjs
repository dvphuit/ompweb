...
test("settles async task history from delivered task-result messages", () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-history-"));
  const sessionFile = join(dir, "sess.jsonl");
  try {
    const lines = [
      ...
      JSON.stringify({
        ...
        message: {
          ...
          details: {
            progress: [
              { id: "AsyncJob", index: 0, agent: "scout", status: "pending", task: "Audit status", tokens: 120 },
              { id: "FailedAsyncJob", index: 1, agent: "worker", status: "running", task: "Run checks" },
            ],
            async: { state: "running", jobId: "AsyncJob", type: "task" },
          },
        },
      }),
      JSON.stringify({
        ...
        content: "<system-notice>\n<task-result id=\"AsyncJob\" agent=\"scout\" status=\"completed\" duration=\"1s\">\n<output>done</output>\n</task-result>\n<task-result id=\"FailedAsyncJob\" agent=\"worker\" status=\"failed (exit 1)\" duration=\"1s\">\n<output>failed</output>\n</task-result>\n</system-notice>",
        display: true,
      }),
    ];
    writeFileSync(sessionFile, lines.join("\n") + "\n");
    ...
    assert.equal(roster[0].task, "Audit status");
    assert.equal(roster[0].tokens, 120);
    assert.equal(roster[0].detached, true);
    assert.equal(roster[1].status, "failed");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("settles consumed async task history from hub job snapshots", () => {
  const dir = mkdtempSync(join(tmpdir(), "ompweb-subagent-history-"));
  const sessionFile = join(dir, "sess.jsonl");
  try {
    const lines = [
      ...
      JSON.stringify({
        ...
        message: {
          ...
          content: [],
          details: {
            progress: [{ id: "FailedJob", index: 0, agent: "worker", status: "running", task: "Run checks" }],
            async: { state: "running", jobId: "FailedJob", type: "task" },
          },
        },
      }),
      JSON.stringify({
        ...
        message: {
          ...
          details: {
            op: "wait",
            jobs: [{ id: "FailedJob", type: "task", status: "failed", label: "FailedJob", durationMs: 2500, resolvedModel: "provider/model" }],
          },
        },
      }),
    ];
    ...
    const roster = extractSubagentHistory(sessionFile);
    assert.equal(roster.length, 1);
    assert.equal(roster[0].status, "failed");
    assert.equal(roster[0].durationMs, 2500);
    assert.equal(roster[0].resolvedModel, "provider/model");
    assert.equal(roster[0].detached, true);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
