...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { models } = await runIsolatedUtilityCommand<{ models: OmpModel[] }>({ type: "get_available_models" }, {
    ...
    if (!found) return NextResponse.json({ ok: false, error: `Model ${providerName}/${modelId} did not resolve — check the API key and provider config`, code: "model_test_unresolved", latencyMs });
    return NextResponse.json({ ok: true, latencyMs, responseText: `${found.provider}/${found.id} validated` });
  } catch (error) {
